<form action="{{ route('admin.categories.update', $category->slug) }}" data-slug="{{ $category->slug }}" id="form-image"
    ...
    <label class="grid group cursor-pointer">
        ...
        <input type="hidden" name="oldImage" value="{{ $category->image }}" class="row-span-full col-span-full">
        <input type="file" class="hidden row-span-full col-span-full" onchange="previewImage(event)" name="image">
        <div
