<div>
        ...
        <ul
            ...
            <li>
                <button
                    ...
                    wire:click="$emit('filter', '')">Semua produk</button>
            </li>
            @foreach ($categories as $category)
                <li>
                    <button
                        class="@if ($categorySlug == $category->slug) bg-secondary text-white @endif h-full p-3 text-base font-normal transition-colors hover:bg-secondary hover:text-white"
                        wire:click="$emit('filter', '{{ $category->slug }}')">{{ $category->name }}</button>
                </li>
            @endforeach
        </ul>
    </div>
    ...
    @if ($products->hasMorePages())
        <button
            class="showMore mt-8 flex w-full flex-col items-center gap-y-2 border-t border-solid border-onPrimary py-3 text-center text-onPrimary"
